...
Select 'Spool '||instance_name||'_Analyzed_'||To_Char(SysDate,'ddmonyy_hh24mi')||'.TXT' from v$instance;
Spool off
@QdAcTrNomeDate.sql
...
select owner, 'not partitioned' x, trunc(last_analyzed) analyzed, count(*)
	from dba_tables
--	where (owner like upper('%PROPRIETARIO%'))
...
Select owner, table_name, 'Heap' type, num_rows, sample_size, sample_size/num_rows*100 percent, last_analyzed
 from dba_tables
--  where  owner not in ('SYSTEM','SYS','IBMDBA','RESTORE','RESTORE2','RESTORE3','DBSNMP')
--	where (owner like upper('%PROPRIETARIO%'))
    and num_rows> 0  and  partitioned = 'NO'
union
Select table_owner, table_name, 'Part', sum(num_rows), sum(sample_size), sum(sample_size)/sum(num_rows)*100,  max(last_analyzed)
	from dba_tab_partitions  
--where table_owner not in ('SYSTEM','SYS','IBMDBA','RESTORE','RESTORE2','RESTORE3','DBSNMP')
   and num_rows> 0  
...
prompt
prompt  Ed analyzed.sql                   @ analyzed.sql
prompt
